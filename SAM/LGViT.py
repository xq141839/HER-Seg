...
class LinearAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, qkv_bias=True, **kwargs):
        ...
        self.up_proj = nn.Linear(dim, int(dim * 0.25))
        self.down_proj = nn.Linear(int(dim * 0.25), dim)
        self.act = nn.SiLU()
    ...
    def forward(self, x):
        ...
        qkv = self.qk(x).reshape(b, n, 2, c).permute(2, 0, 1, 3)
        q, k, v = qkv[0], qkv[1], x
        v = self.up_proj(v)
        v = self.act(v)
        v = self.down_proj(v)
...
class BasicLayer(nn.Module):
    """ A basic LGViT layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            LGViTBlock(dim=dim, input_resolution=input_resolution, num_heads=num_heads,
...
class LGViT(nn.Module):
    ...
    Args:
        ...
        depths (tuple(int)): Depth of each LGViT layer.
    ...
    def __init__(self, img_size=1024, patch_size=16, in_chans=3, embed_dim=192, 
                 depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 mlp_ratio=2., qkv_bias=True, drop_rate=0., drop_path_rate=0.1,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                layer = ConvLayer(dim=embed_dim, depth=depths[i_layer], activation=nn.GELU, conv_expand_ratio=self.mlp_ratio)
            else:
                layer = BasicLayer(dim=embed_dim,
                                input_resolution=(patches_resolution, # patches_resolution[0] // (2 ** i_layer
                                                    patches_resolution), # patches_resolution[1] // (2 ** i_layer
    ...
    def forward_features(self, x):
        ...
        for idx, layer in enumerate(self.layers):
            if idx == 0:
                x = x.view(b,w,h,d).permute(0, 3, 1, 2) # b d w h
                x = layer(x)
                x = x.permute(0, 2, 3, 1).view(b,w*h,d)
            else:
                x = layer(x)
